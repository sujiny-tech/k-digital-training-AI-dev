...
+ 퍼셉트론 학습 알고리즘
   + stochastic version
      ...
      + 실험으로 더 좋은 성능이 입증되서 현대 기계학습은 이 버전을 채택하였음.
      
   + batch version
      + batch로 확장해도 동일(틀린샘플을 모두 모아 한꺼번에 매개변수 갱신)
   
   
   + 선형분류기(linear classifier) 한계
...
+ 핵심 아이디어
   + 은닉층
      > 원래 특징 곤가을 분류하는 데 훨씬 **유리한 새로운 특징 공간**으로 변환
   ...
   + 오류 역전파 알고리즘 사용
      > 역방향으로 진행하면서 한번에 한층씩 gradient 계산하고 가중치 갱신하는 알고리즘
